            ...
            resp => {
                ...
                if (error.status === 401) {
                    ...
                    if (errorMessage.toLowerCase().includes('user is disabled')) {
                        ...
                        this.messageService.add({severity: 'error', summary: 'Error ' + error.status, detail: 'Invalid credentials'});
                    }
        ...
        this.http.post<any>(this.API + 'register', this.user, {observe: 'response'}).subscribe(
            resp => {
                ...
                if (error.status === 401) {
                    ...
                    this.messageService.add({severity: 'error', summary: 'Error ' + error.status, detail: error.error.error});
                }
            }
        );
    }
